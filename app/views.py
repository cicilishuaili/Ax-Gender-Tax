...
def results(ad_hash):
    ...
    notes_f = Label(x=min_x-16, y=max_y-0.18,text='More Likely Female',text_color='#A9A9A9')
    notes_x = Label(x=min_x-20.5, y=max_y-0.39,text='Higher Cost',text_align='right',angle=np.pi/2,text_color='#A9A9A9')
    notes_y = Label(x=min_x-19.5, y=min_y,text='Lower Cost',angle=np.pi/2,text_color='#A9A9A9')
    notes_m = Label(x=max_x-2, y=max_y-0.18,text='More Likely Male',text_align='right',text_color='#A9A9A9')
